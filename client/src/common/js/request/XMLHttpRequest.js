...
class XHR {
    ...
    open () {
        ...
        this.xmlHttp.open(method, method == 'GET'? url+'?'+this.queryStringify(data):url, async);
    }
    // 设置请求头
    ...
    // 发送数据
    send () {
        let {data={},method}=this.options;
        //  data  instanceof  FormData ?this.queryStringify(data):url
        if( !(data  instanceof  FormData)){
            data=this.queryStringify(data)
        }
        // const keys = Object.keys(data);
